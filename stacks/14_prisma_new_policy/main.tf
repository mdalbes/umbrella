...
module "policy_1" {
  ...
  query             = "config from cloud.resource where cloud.account = 'acg-umbrella-6260' and api.name = 'aws-ec2-describe-security-groups' AND json.rule = ipPermissions[*].toPort is not member of (443,80)"
  unit              = module.variables.unit_1
  amount            = module.variables.amount_1
  savedsearch_name  = "saved-search-umbrella-6260"
  policy_name       = "policy-umbrella-6260"
  rule_name         = "rule-name-umbrella-6260"
